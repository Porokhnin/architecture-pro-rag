...
def has_files_changed_yesterday(folder_path):
    """
    Checks if any file in the specified folder was modified yesterday.

    Args:
        folder_path (str): The path to the directory to check.

    Returns:
        bool: True if at least one file was modified yesterday, False otherwise.
    """
    # 1. Calculate yesterday's date (as a date object, without time)
    today = datetime.now().date()
    yesterday = today - timedelta(days=1)

    # 2. Iterate over all entries in the directory
    for entry in os.scandir(folder_path):
        if entry.is_file():
            ...
            # 4. Convert the timestamp to a datetime object and extract the date part
            mtime_date = datetime.fromtimestamp(mtime_timestamp).date()

            # 5. Compare the file's modification date with yesterday's date
            if mtime_date == yesterday:
                logger.info(f"File '{entry.name}' was modified yesterday.")
                return True # Found a file modified yesterday, can stop checking

    # If the loop finishes without finding any matching files
    return False


minute = 0
def refresh_vector_index_job():
    global minute

    logger.info(f"refresh_vector_index_job запущен {minute} в {datetime.now()}!")

    try:
        if has_files_changed_yesterday(KNOWLEDGE_BASE_DIR):
            logger.info("Content has changed yesterday. Updating vector index...")
            generate_vector_index()

            scheduler.reschedule_job(refresh_vector_index_job, trigger='cron', hour=2, minute=0)
        else:
            logger.info("No changes detected yesterday. Vector index update not required.")
    except Exception as e:
        logger.error(e)
        if(minute < 59):
            minute += 1
            scheduler.reschedule_job(refresh_vector_index_job, trigger='cron', hour=2, minute=minute)
        else:
            scheduler.reschedule_job(refresh_vector_index_job, trigger='cron', hour=2, minute=0)


# Добавляем задание: каждую ночь в 02:00
scheduler = BackgroundScheduler()
scheduler.add_job(refresh_vector_index_job, trigger='cron', hour=2, minute=0)
scheduler.start()
